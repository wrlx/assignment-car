 ...
 * Use the [ManufactureFragment.newInstance] factory method to
...
class ManufactureFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_manufacture, container, false)
        ...
        var viewModel= ViewModelProvider(this).get(ManufactureViewModel::class.java)

        viewModel.carDetails.observe(viewLifecycleOwner){
            Log.d("GET CAR DATA", it.carList.first().carManufactureName)

            val adapter = CarAdapter(it.carList, { onItemClickListener(it) })
